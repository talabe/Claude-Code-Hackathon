...
const ChangeLogWrapper = ({ children }: { children: React.ReactNode }) => {
  ...
  const sidebarData = Array.from(
    getSideBarData.props.value.matchAll(
      /<div\s+class="changelog[^"]*"\s+id="changelog"\s+data-version="([^"]+)"\s+data-date="([^"]+)"\s+data-status="([^"]+)"\s+data-status-value="([^"]+)"[^>]*>(.*?)<\/div>/g,
    ),
    ...
    <section>
      <div className="container">
        <div className="row">
          <div className="hidden pt-8 lg:col-3 md:pr-10 md:pt-12 lg:block">
            <div className="changelog-link sticky top-[149px]">
              {sidebarData.map((item) => (
                <div
                  key={item.version}
                  className="relative block border-b border-border py-4 text-h5-md font-medium text-text first:pt-0 last-of-type:border-b-0 md:py-7 md:text-h5"
                >
                  <a
          ...
          <div className="pt-12 lg:col-9 lg:border-l lg:border-border lg:pl-10">
            {children}
          </div>
        </div>
      </div>
