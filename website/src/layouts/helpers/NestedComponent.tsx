...
const NestedComponent: React.FC<SidebarProps> = ({
        ...
        {pages?.map((page, index) => (
          <li key={index}>
            <a
              ...
              className={` hover:border-dark block border-l border-border py-[.5rem] pl-4 text-left font-text-light text-text transition-colors duration-300 hover:text-text sm:py-2.5  ${
                regular.includes(`${parentPath}/${page}`)
                  ? "border-dark border-l py-1 pl-4 font-medium text-text sm:py-3"
                  : null
              }`}
            >
              {page}
      ...
      return (
        <React.Fragment key={page.root}>
          <ul className={"mt-4 group-[.active]:max-h-[1000px]"}>
            <li>
              <button
                className={`has-sibling-child flex items-center hover:border-dark border-l capitalize border-border pl-3 text-left font-text-light text-text transition-colors duration-300 hover:text-text ${
                  activeChild.includes(`${parentPath}/${page.root}`)
                    ? "border-dark border-l font-medium text-text "
                    : null
                }`}
  ...
  const getTitle = (root: string) => {
    const foundItem = pageData.find((item: any) => item.id.includes(root));

    return foundItem?.data?.title;
  };
      ...
      {menuData?.map((menuItem, index) => (
        <li key={index} className="active group relative">
          ...
          >
            ...
            <div
              className={`order-0 flex h-[40px] w-[40px] items-center justify-center group-hover:bg-primary rounded-md bg-light p-[10px] transition-colors duration-300  sm:h-[50px] sm:w-[50px] ${
